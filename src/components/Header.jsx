    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-shadow bg-white/90 backdrop-blur dark:bg-neutral-900/90 ${scrolled ? 'shadow-sm' : ''}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <a href="/" className="flex items-center gap-2 select-none">
          <span className="text-2xl font-black tracking-tight text-gray-900 dark:text-white">Neot</span>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a href="#start" className="px-4 py-2 rounded-full border border-[#66BB6A] text-[#66BB6A] hover:bg-[#66BB6A] hover:text-white transition-colors text-sm font-semibold">Get Started</a>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-gray-100 dark:border-neutral-800 bg-white dark:bg-neutral-900">
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-4">
            <a onClick={() => setOpen(false)} href="/" className={navItemClass}>Home</a>
            ...
            <a onClick={() => setOpen(false)} href="#start" className="px-4 py-2 w-full text-center rounded-full border border-[#66BB6A] text-[#66BB6A] hover:bg-[#66BB6A] hover:text-white transition-colors text-sm font-semibold">Get Started</a>
          </div>
        </div>
      )}
    </header>
